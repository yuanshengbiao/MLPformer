...
from layers.Embed1 import PatchEmbedding
...
class T_Block(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        x_enc = x_enc - means
        stdev = torch.sqrt(torch.var(x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5) #计算标准差
        x_enc /= stdev   #标准化操作
...
class F_Block(nn.Module):
    ...
    def forward(self, x_enc, x_enc_mark, x_dec, x_dec_mark):
        paddings = torch.zeros((x_enc.shape[0], self.pred_len, x_enc.shape[2], x_enc.shape[3])).to(x_enc.device)
        x_enc = torch.cat((x_enc, paddings), dim=1)
...
class Model(nn.Module): 
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        _freq = freq.clone()
        _freq[:, :3, :] = 0
        _freq[:, freq.shape[1] // self.harmony_num:, :] = 0  #通过将 _freq 张量中部分频率分量设为 0，来过滤掉特定的频率成分，以实现信号的优化或改进。
